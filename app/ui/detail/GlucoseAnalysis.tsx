...
export default function GlucoseAnalysis(props: GlucoseAnalysisProps) {
    ...
    const avgGlucose = glucoseData.length ? averageGlucose(glucoseValue) : '---';

    const tir = glucoseData.length ? tirGlucose(glucoseValue) : '---';
        ...
        <React.Fragment>
            ...
            <div className='px-4 flex flex-col items-center lg:justify-between lg:flex-row lg:px-10'>
                <Box className="w-[80%] lg:min-w-[25%] lg:max-w-[33%]">
                    <Card variant="outlined"><GlucoseDataCard value={glucoseValue.length === 1 && glucoseValue[0] === 0 ? '---' : '' + glucoseValue[glucoseValue.length - 1]} text="Latest glucose data" type="latest" /></Card>
                </Box>
                <Box className="w-[80%] lg:min-w-[25%] lg:max-w-[33%]">
